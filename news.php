...
if (isset($_GET['id'])) {
    ...
    // Check if a user has already viewed this article in the current session
    ...
    if (!in_array($article_id, $_SESSION['viewed_articles'])) {
        ...
        $updateViewCountQuery = "UPDATE article SET view_count = view_count + 1 WHERE article_id = {$article_id}";
        $updateViewCountResult = mysqli_query($con, $updateViewCountQuery);

        if (!$updateViewCountResult) {
            die("Database query failed: " . mysqli_error($con));
        }

        // Mark this article as viewed in the current session
        $_SESSION['viewed_articles'][] = $article_id;
    }
} else {
    // If we don't get article_id from URL, redirect to home page
    redirect('index.php');
}

// Article Query to fetch all data related to a particular article
$articleQuery = " SELECT category.category_name, category.category_id, category.category_color, article.*, author.*, article.view_count
  ...
  <div class="container">
    <div class="page-container">
      ...
      <article class="card1">
        ...
        <!--<figcaption><?php echo $ficap; ?></figcaption>-->
        ...
        <div class="meta">
          ...
          <!-- Article view count -->
        <div class="author">
          <i class="fa fa-eye"></i> <?php echo $data['view_count']; ?>
       </div>
          ...
          <div class="date">
            ...
            <?php
              echo date("d M Y H:m",$article_date);
            ?>
          </div>
    ...
    // Share button
    echo '<button class="btn btn-download" onclick="shareArticle(\'' . $article_title . '\', \'' . $article_image . '\')">Share to 5 groups $ earn Ksh.50 &nbsp<i class="fas fa-share"></i></button>';
    ?>
</div>
...
<script>
    function shareArticle(title, image) {
        // Construct the shareable URL (you can adjust this as needed)
        const shareUrl = window.location.href;
        // Create a shareable text with the article title and image
        const shareText = `Trending articles: ${title}`;

        // Open the share dialog (you can use a custom share dialog or the browser's built-in dialog)
        if (navigator.share) {
            ...
            // You can implement your own custom share dialog here or redirect to a sharing page
            // For simplicity, we'll use a basic prompt
            prompt('Share this article:', shareText + '\n' + shareUrl);
        }
    }
</script>
